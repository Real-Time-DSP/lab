...
% Click Run at the top or type 'receiver_demo' in the command window
classdef receiver_demo < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function run_receiver(app, event)
            ...
            demodulated = r .* cos(w0*(n + carrier_offset));
            recovered_pulses = conv(demodulated,pulse_shaping_filter);
            recovered_pulses(1:(group_delay + symbol_offset)) = [];
            
            recovered_data = downsample(recovered_pulses,N_symbol) > 0;
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.VariableEditField.Value = 'not_scrambled';
